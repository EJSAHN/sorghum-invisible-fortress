...
Summarize effect-sign quadrant enrichment for paired SDI and disease GWAS results.

Inputs expected, generated by sdi_pipeline.py:
- <out>/GWAS_SDI.csv
- <out>/GWAS_<disease_trait>.csv

Outputs written to <out>/addons_effect_sign/:
- effect_sign_enrichment_summary.csv
- effect_sign_allelefreq_effectsize_by_quadrant.csv
- plots/effect_sign_scatter_<trait>_top<xx>.pdf   (if --make_plots)
...
def normalize_gwas_columns(df: pd.DataFrame, *, path_label: str) -> pd.DataFrame:
    """Return a GWAS table with chr, pos, beta, logp, and optional maf columns."""
    out = df.copy()
    if "logp" not in out.columns:
        if "-log10p" in out.columns:
            out = out.rename(columns={"-log10p": "logp"})
        elif "p" in out.columns:
            p = np.clip(out["p"].to_numpy(dtype=float), 1e-300, 1.0)
            out["logp"] = -np.log10(p)
...
def classify_quadrant(beta_sdi: np.ndarray, beta_dis: np.ndarray) -> np.ndarray:
    """Assign formal effect-sign quadrants based on paired beta coefficients."""
...
def quadrant_directions(q: str) -> Tuple[str, str]:
    mapping = {
        "Qpp": ("positive", "positive"),
        "Qpn": ("positive", "negative"),
        "Qnp": ("negative", "positive"),
        "Qnn": ("negative", "negative"),
    }
    return mapping.get(q, ("", ""))


def fisher_enrichment(sig_mask: np.ndarray, class_mask: np.ndarray) -> Tuple[float, float]:
    """Fisher exact test for enrichment of a quadrant among selected SNPs."""
...
def main() -> None:
    ...
    ap.add_argument("--out", type=str, default="outputs", help="Output folder from sdi_pipeline.py")
    ap.add_argument(
        "--disease_pattern",
        type=str,
        default="GWAS_headsmut*.csv",
    ...
    ap.add_argument("--use_fdr", action="store_true", help="Also compute BH-FDR q-values and report q<0.1 enrichment")
    ...
    if not gwas_sdi_path.exists():
        raise FileNotFoundError(f"Missing {gwas_sdi_path}. Run sdi_pipeline.py first.")
    ...
    addon_dir = out_root / "addons_effect_sign"
    plot_dir = addon_dir / "plots"
    addon_dir.mkdir(parents=True, exist_ok=True)
    plot_dir.mkdir(parents=True, exist_ok=True)

    gwas_sdi = normalize_gwas_columns(pd.read_csv(gwas_sdi_path), path_label=str(gwas_sdi_path))
    ...
    for f in disease_files:
        ...
        gwas_dis = normalize_gwas_columns(pd.read_csv(f), path_label=f)
        ...
        for pct in top_pcts:
            ...
            for qd in quads:
                ...
                sdi_direction, disease_direction = quadrant_directions(qd)
                detail_rows.append({
                    "trait": trait,
                    "threshold_type": f"top{pct:.3g}pct_logp",
                    "logp_threshold": thr,
                    "quadrant": qd,
                    "sdi_effect_direction": sdi_direction,
                    "disease_effect_direction": disease_direction,
                    "n_sig": int(sig.sum()),
            ...
            if args.make_plots:
                ...
                fig.tight_layout()
                fig.savefig(plot_dir / f"effect_sign_scatter_{trait}_top{pct:g}pct.pdf")
                plt.close(fig)
    ...
    summary_df.to_csv(addon_dir / "effect_sign_enrichment_summary.csv", index=False)
    detail_df.to_csv(addon_dir / "effect_sign_allelefreq_effectsize_by_quadrant.csv", index=False)
