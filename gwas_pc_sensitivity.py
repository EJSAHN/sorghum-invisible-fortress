...
Quantify sensitivity of GWAS results to population-structure controls:
- number of genotype PCs (0..K)
- optional race dummy covariates (if present)

This module reports lead-locus stability and genomic inflation across covariate settings.
...
Note:
This script does not replace the main pipeline. It reuses the cached genotype matrix
...
def gwas_ols(y: np.ndarray, G_int8: np.ndarray, meta: pd.DataFrame, cov: Optional[np.ndarray], block: int = 20000) -> Tuple[pd.DataFrame, float, Dict]:
    """
    Copy of sdi_pipeline.py gwas_ols (OLS GWAS w/ covariates).
    """
...
def main() -> None:
    ...
    if not cache_dir.exists():
        raise FileNotFoundError(f"Missing cache dir: {cache_dir}. Run sdi_pipeline.py first.")
