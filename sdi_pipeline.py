...
# 1) Run `sdi_pipeline.py` in the directory containing the raw inputs to reproduce the main GWAS / SDI / effect-sign results and Supplementary_Data_S1.xlsx.
# 2) (Optional) Run `candidate_gene_keyword_enrichment.py --out outputs --resources .` to generate GO_Enrichment_CandidateGenes.csv and keyword enrichment tables, and to append them as new sheets to Supplementary_Data_S1_with_GO.xlsx.
# Core steps:
#  - Vis/NIR PC1 + SDI
#  - HapMap -> dosage int8 -> QC
#  - OLS GWAS with PCs (+race if present)
#  - Disease traits (highest score + greenhouse average, when available)
#  - Candidate SNP annotation via GFF + P14 annotation_info/defline if present
# ============================================================

from __future__ import annotations
import os, re, json, gzip, glob, tempfile, warnings, argparse
...
def main():
    ...
    hapmap_path = _find_first(["*_geno_aligned.hmp.txt", "*.hmp.txt", "*.hmp*.txt"])
    ...
    for tr in disease_traits:
        ...
        top_n = max(1, int(len(merged) * TOP_PCT_FOR_EFFECT_SIGN))
        ...
        def cls(row):
            ...
            if (not np.isfinite(bd)) or (abs(bd) < cut):
                return "Low_effect"
            if bs > 0 and bd > 0: return "Qpp"
            if bs > 0 and bd < 0: return "Qpn"
            if bs < 0 and bd < 0: return "Qnn"
            if bs < 0 and bd > 0: return "Qnp"
            return "Mixed"
        hits["effect_quadrant"] = hits.apply(cls, axis=1)

        effect_hits_map[tr] = hits
        effect_sum_map[tr] = hits.groupby("effect_quadrant").size().reset_index(name="n")
    ...
    for tr in disease_traits:
        ...
        effect_hits_map[tr].to_csv(os.path.join(OUT_DIR, f"EffectSign_{trait_short(tr)}.csv"), index=False)
    ...
    with pd.ExcelWriter(s1_path, engine="openpyxl") as xl:
        ...
        for tr in disease_traits:
            ...
            effect_hits_map[tr].to_excel(xl, sheet_name=_sheet(f"S1_EffectSign_{sh}"), index=False)
    ...
    print("[DONE] SDI pipeline complete. Outputs ->", OUT_DIR)
