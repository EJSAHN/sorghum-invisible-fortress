...
Robustness checks for the sorghum SDI hyperspectral pipeline.

What it does (no new wet-lab data required):
1) Re-states SDI construction from the exported phenotype table.
2) Computes SDI ↔ disease associations under multiple confounding controls:
   - unadjusted
...
5) SDI threshold sensitivity and data-driven threshold scan
...
This script uses CLI arguments only and does not require hard-coded personal paths.
"""
...
def main() -> None:
    ...
    ap.add_argument("--sdi_threshold", type=float, default=7.0, help="Specific SDI threshold to include in the threshold scan")
    ...
    if not pheno_path.exists():
        raise FileNotFoundError(f"Missing {pheno_path}. Run sdi_pipeline.py first.")
    ...
    # 4) Threshold scan (quantile grid + optional specified threshold)
    ...
    candidates = sorted(set([float(args.sdi_threshold)] + [float(q) for q in quantiles]))
    thr_rows = []
    for tr in trait_cols:
        thr_df = threshold_scan(df, sdi_col=args.sdi_col, trait=tr, candidate_thresholds=candidates)
        if len(thr_df) == 0:
            continue
        thr_df["specified_threshold"] = float(args.sdi_threshold)
    ...
    if args.make_plots:
        ...
        if not thr_all.empty:
            for tr in trait_cols:
                ...
                ax.axvline(float(args.sdi_threshold), linestyle="--")
